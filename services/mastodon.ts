...
export class MastodonService {
  ...
  async lookupAccount(username: string): Promise<MastodonAccount> {
    return this.fetchApi<MastodonAccount>('accounts/lookup', { acct: username });
  }

  /**
   * 获取原始分页数据。
   */
  async getStatusesPage(accountId: string, maxId?: string, limit: number = 40): Promise<MastodonStatus[]> {
    const params: Record<string, string> = { 
      limit: limit.toString(),
    };
    if (maxId) params.max_id = maxId;
    return this.fetchApi<MastodonStatus[]>(`accounts/${accountId}/statuses`, params);
  }
  ...
  async getStatusesUntil(
  ...
  ): Promise<{ statuses: MastodonStatus[], lastId?: string, reachedThreshold: boolean }> {
    ...
    while (hasMore && !reachedThreshold) {
      const batch = await this.getStatusesPage(accountId, maxId);
    ...
    return { statuses: allFilteredStatuses, lastId: maxId, reachedThreshold };
  }

  async searchStatuses(query: string, username: string, limit: number = 20): Promise<MastodonStatus[]> {
